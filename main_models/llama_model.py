...
class LlamaModel(LLM):
    ...
    def __init__(self, model_name_or_path: str, device: str="cuda", cpu_offloading: bool=False) -> None:
        ...
        self.tokenizer = LlamaTokenizer.from_pretrained(model_name_or_path, use_fast=False)
        self.model = LlamaForCausalLM.from_pretrained(
            model_name_or_path, torch_dtype=torch.float16, low_cpu_mem_usage=True) #load_in_4bit=True,
        print("LlamaModel loaded base model from {}".format(model_name_or_path)
        )
        '''
