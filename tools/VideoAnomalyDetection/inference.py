...
mllm_path = os.path.join(VTO_ROOT, "tools/HolmesVAU/ckpt")
sampler_path = os.path.join(VTO_ROOT, "tools/HolmesVAU/holmesvau/ATS/anomaly_scorer.pth")
device = torch.device('cuda:0')
model, tokenizer, generation_config, sampler = load_model(mllm_path, sampler_path, device)

video_path = os.path.join(VTO_ROOT, "tools/HolmesVAU/examples/robbery.mp4")
